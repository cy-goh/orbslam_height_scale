...
void ScaleManager::manageScale(ORB_SLAM2::Frame &f1, ORB_SLAM2::Frame &f2, cv::Mat &img1, cv::Mat &img2, int frame_id, cv::Mat &refR, cv::Mat &t, cv::Mat &desc1, cv::Mat &desc2, std::vector<cv::KeyPoint> &kp1, std::vector<cv::KeyPoint> &kp2, bool &rescale, float &estimated_scale)
{
    ...
    float scale = sparse_est->calculate_scale(desc1, desc2, kp1, kp2, 3 * 100, 0.60, est_norm, inlier_ratio);
    ...
    {
        cout << "Bad sparse output: (angle)" << angle << " (scale): " << scale << endl;
        rescale = false;
        estimated_scale = -1;
        qAppend(estimated_scale);
        return;
    ...
    if (avg_angle < 7.5)
    {
        ...
        cout << "running average of " << m_count << " with scale of " << 1.7 * cv::norm(avg_norm) << "org:" << 1.7 * cv::norm(est_norm) <<  endl;
    }
    else
    {
        // est_norm /= cv::norm(t);
    ...
    try
    {
        ...
        final_scale = dense_est->optimize_scale(refR, t, n1, n2, n3, 0, 30, img1f, img2f, final_norm, false);
    }
    ...
    if (dense_angle < 20)
    {
        cout << "good scale at " << final_scale * cv::norm(t) << endl;
        estimated_scale = final_scale;
        qAppend(estimated_scale * cv::norm(t));
