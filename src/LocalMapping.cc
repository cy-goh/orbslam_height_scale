...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    // if (mpCurrentKeyFrame->mTimeStamp - mTimeStampLastUpdate > SCALE_UPDATE_PERIOD)
    if (true)
    {
        //TODO: renable this
        Rescale(mpCurrentKeyFrame);
        mTimeStampLastUpdate = mpCurrentKeyFrame->mTimeStamp;
    }
}
...
void LocalMapping::Rescale(KeyFrame *targetKF)
{
    using namespace cv;
    ...
    mLastAddedScale = scale;

    vector<KeyFrame *> localKF;
    localKF.push_back(targetKF);

    const vector<KeyFrame *> covisKF = targetKF->GetVectorCovisibleKeyFrames();
    int maxIdToCorrect = 0;
    ...
    for (auto pKFi : covisKF)
    {
        ...
        // if (!pKFi->isBad() && pKFi->mnId > mLastCorrectedId)
        {
            ...
            if (pKFi->mnId > maxIdToCorrect)
                maxIdToCorrect = pKFi->mnId;
        }
    }

    if (localKF.size() < 2)
    {
        cout << "NOT ENOUGH KF TO SCALE. target : " << targetKF->mnFrameId << endl;
        return;
    }
    ...
    sort(sortedLocalKF.begin(), sortedLocalKF.end(), KeyFrame::lId);
    // vector<KeyFrame*> sortedLocalKF = localKF;

    for (auto kf: sortedLocalKF)
    {
        cout << kf->mnFrameId << " ";
    }
    ...
    KeyFrame *firstFrame = sortedLocalKF[0];
    Mat Twf = firstFrame->GetPoseInverse();
    Mat Tfw = firstFrame->GetPose();
    Mat Rfw = Tfw.rowRange(0, 3).colRange(0, 3);
    Mat tfw = Tfw.rowRange(0, 3).col(3);

    g2o::Sim3 g2oSfw(Converter::toMatrix3d(Rfw), Converter::toVector3d(tfw), 1.);

    for (int i = 0; i < sortedLocalKF.size(); i++)
    {
        KeyFrame *kfi = sortedLocalKF[i];

        Mat Tiw = kfi->GetPose();
        Mat Tif = Tiw * Twf; //Tiw * Twc
        Mat Rif = Tif.rowRange(0, 3).colRange(0, 3);
        Mat tif = Tif.rowRange(0, 3).col(3);
        ...
        kfi->SetPose(TiwCorrected);

        // cv::Mat t = sortedLocalKF[i]->GetCameraCenter();
        // afterFile << t.at<float>(0) << "," << t.at<float>(1) << "," << t.at<float>(2) << endl;

        // sortedLocalKF[i]->UpdateConnections();
        ...
        vector<MapPoint *> mapPointsMatches = kfi->GetMapPointMatches();
        ...
        for (size_t indexMP = 0; indexMP < mapPointsMatches.size(); indexMP++)
        {
            ...
            if (!mapPointi || mapPointi->isBad())
                continue;
            if (mapPointi->mnCorrectedByKF == targetKF->mnId)
                continue;
            ...
            mapPointi->mnCorrectedByKF = targetKF->mnId;
            mapPointi->mnCorrectedReference = kfi->mnId;
